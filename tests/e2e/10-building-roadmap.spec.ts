...
test.describe("建設ロードマップ機能", () => {
  ...
  test.describe("1. 基本表示とナビゲーション", () => {
    test("1.1 初回アクセス時の表示", async ({ page }) => {
      // 電磁マトリックスを選択
      ...
      // 全体進捗が0%で表示される
      await expect(page.getByText(/全体進捗:\s*0%/)).toBeVisible();

      // 原材料採掘フェーズが展開されている
      const rawMaterialsPhase = page.getByRole("button", { name: /▼\s*原材料採掘/ });
      await expect(rawMaterialsPhase).toBeVisible();

      // Phase 2以降が折りたたまれている
      const phase2 = page.getByRole("button", { name: /▶\s*Phase 2:/ });
    ...
    test("1.2 タブ間の切り替え", async ({ page }) => {
      ...
      await expect(page.getByTestId("recipe-node-1801")).toBeVisible();
    ...
    test("1.3 異なるレシピでの表示", async ({ page }) => {
      // 鉄インゴット（シンプル）を選択
      await page.getByTestId("recipe-button-1101").click();
      await page.getByTestId("roadmap-tab").click();

      // フェーズが少ないことを確認（2-3フェーズ程度）
      const ironPhases = await page.getByRole("button", { name: /Phase \d+:/ }).count();
      ...
      const structurePhases = await page.getByRole("button", { name: /Phase \d+:/ }).count();
  ...
  test.describe("2. フェーズ管理機能", () => {
    ...
    test("2.1 フェーズの展開と折りたたみ", async ({ page }) => {
      // Phase 2が折りたたまれていることを確認
      const phase2Button = page.getByRole("button", { name: /▶.*Phase 2:/ });
      ...
      const magnetNode = page.getByRole("button", { name: /☐.*磁石/ });
    ...
    test("2.2 複数フェーズの同時展開", async ({ page }) => {
      // Phase 2, 3, 4を順次展開
      await page.getByRole("button", { name: /▶.*Phase 2:/ }).click();
      await page.getByRole("button", { name: /▶.*Phase 3:/ }).click();
      await page.getByRole("button", { name: /▶.*Phase 4:/ }).click();

      // すべて展開されていることを確認
      await expect(page.getByRole("button", { name: /▼.*Phase 2:/ })).toBeVisible();
      await expect(page.getByRole("button", { name: /▼.*Phase 3:/ })).toBeVisible();
    ...
    test("2.3 フェーズタイトルの適切性", async ({ page }) => {
      // Phase 1が原材料採掘であることを確認
      await expect(page.getByText(/原材料採掘/)).toBeVisible();

      // Phase 2のタイトルに生産物名が含まれることを確認
      await expect(page.getByText(/Phase 2:.*1次加工/)).toBeVisible();

      // Phase 3のタイトルに中間製品名が含まれることを確認
      await expect(page.getByText(/Phase 3:.*2次加工/)).toBeVisible();

      // 最終フェーズに「最終完成品」が含まれることを確認
      await expect(page.getByText(/最終完成品/)).toBeVisible();
    });
  });
  ...
  test.describe("3. チェックリスト機能", () => {
    ...
    test("3.1 個別ノードのチェック/アンチェック", async ({ page }) => {
      // 初期状態で全体進捗が0%
      await expect(page.getByText(/全体進捗:\s*0%/)).toBeVisible();

      // 鉄鉱石ノードをクリック
      const ironOreNode = page.getByRole("button", { name: /☐\s*鉄鉱石/ });
      await ironOreNode.click();

      // チェックマークが表示される
      await expect(page.getByRole("button", { name: /☑\s*鉄鉱石/ })).toBeVisible();

      // 全体進捗が更新される（0%より大きい）
      const progressText = await page.getByText(/全体進捗:\s*\d+%/).innerText();
      ...
      // フェーズ進捗が更新される
      await expect(page.getByText(/1\/2\s*完了/)).toBeVisible();

      // 再度クリックしてチェックを外す
      await page.getByRole("button", { name: /☑\s*鉄鉱石/ }).click();
    ...
    test("3.2 複数ノードの連続チェック", async ({ page }) => {
      // 鉄鉱石をチェック
      await page.getByRole("button", { name: /☐\s*鉄鉱石/ }).click();

      // 銅鉱石をチェック
      await page.getByRole("button", { name: /☐\s*銅鉱石/ }).click();

      // Phase 2を展開
      await page.getByRole("button", { name: /▶\s*Phase 2:/ }).click();

      // 磁石をチェック
      await page.getByRole("button", { name: /☐\s*磁石/ }).click();

      // 銅インゴットをチェック
      await page.getByRole("button", { name: /☐\s*銅インゴット/ }).click();
      ...
      // Phase 2の進捗を確認
      await expect(page.getByText(/2\/3\s*完了/)).toBeVisible();
    });

    test("3.3 フェーズ完了時の動作", async ({ page }) => {
      // 原材料採掘フェーズのすべてのノードをチェック
      await page.getByRole("button", { name: /☐\s*鉄鉱石/ }).click();
      await page.getByRole("button", { name: /☐\s*銅鉱石/ }).click();

      // フェーズ進捗が完了状態になる
      await expect(page.getByText(/2\/2\s*完了/)).toBeVisible();
    ...
    test("3.4 ノード表示内容の確認", async ({ page }) => {
      // 原材料ノードの確認（鉄鉱石）
      const ironOreNode = page.getByRole("button", { name: /☐.*鉄鉱石/ }).first();
      ...
      const magnetNode = page.getByRole("button", { name: /☐.*磁石.*アーク製錬所/ }).first();
  ...
  test.describe("4. フェーズ一括操作", () => {
    ...
    test("4.1 フェーズ内全ノードの一括チェック", async ({ page }) => {
      // Phase 2を展開
      await page.getByTestId("phase-header-2").click();

      // すべて切替ボタンをクリック（data-testidを使用）
      const toggleAllButton = page.getByTestId("phase-toggle-all-2");
      await toggleAllButton.click();

      // 全体進捗が更新される
      const progressText = await page.getByText(/全体進捗/).innerText();
      const progressMatch = progressText.match(/(\d+)%/);
      expect(progressMatch).toBeTruthy();
      const progress = parseInt(progressMatch![1]);
      expect(progress).toBeGreaterThan(0);
      ...
      // 全体進捗が下がることを確認
      const newProgressText = await page.getByText(/全体進捗/).innerText();
    ...
    test("4.2 複数フェーズでの一括操作", async ({ page }) => {
      // 原材料採掘の一括チェック（data-testidを使用）
      const rawMaterialsToggle = page.getByTestId("phase-toggle-all-1");
      await rawMaterialsToggle.click();

      // Phase 2を展開して一括チェック
      await page.getByTestId("phase-header-2").click();
      const phase2Toggle = page.getByTestId("phase-toggle-all-2");
  ...
  test.describe("5. 進捗表示機能", () => {
    ...
    test("5.1 全体進捗の表示", async ({ page }) => {
      ...
      let progressText = await page.getByText(/全体進捗:\s*\d+%/).innerText();
      ...
      progressText = await page.getByText(/全体進捗:\s*\d+%/).innerText();
    ...
    test("5.2 フェーズ別進捗の表示", async ({ page }) => {
      // 原材料採掘フェーズのヘッダーを取得
      const rawMaterialsHeader = page.getByRole("button", { name: /▼.*原材料採掘/ });
      ...
      // Phase 2の進捗を確認
      const phase2Header = page.getByRole("button", { name: /▶.*Phase 2:/ });
  ...
  test.describe("6. リセット機能", () => {
    ...
    test("6.1 全体リセット", async ({ page }) => {
      ...
      // 進捗が0%でないことを確認
      let progressText = await page.getByText(/全体進捗/).innerText();
      const progressMatch = progressText.match(/(\d+)%/);
      const progress = parseInt(progressMatch![1]);
      expect(progress).toBeGreaterThan(0);
      ...
      // すべてリセットボタンをクリック
      const resetButton = page.getByRole("button", { name: "すべてリセット" });
      ...
      // 進捗が0%に戻る
      progressText = await page.getByText(/全体進捗/).innerText();
    ...
    test("6.2 リセット後の再チェック", async ({ page }) => {
      // チェック → リセット
      await page.getByRole("button", { name: /☐.*鉄鉱石/ }).click();
  ...
  test.describe("7. データ永続化", () => {
    test.skip("7.1 ローカルストレージへの保存とページリロード後の復元", async ({ page }) => {
      ...
      // 全体進捗を記録
      const beforeReloadProgress = await page.getByText(/全体進捗/).innerText();
      ...
      // 全体進捗が同じ
      const afterReloadProgress = await page.getByText(/全体進捗/).innerText();
  ...
  test.describe("8. UI/UXテスト", () => {
    ...
    test("8.1 レスポンシブデザイン - デスクトップ", async ({ page }) => {
      // デスクトップサイズに設定
    ...
    test("8.2 レスポンシブデザイン - タブレット", async ({ page }) => {
      // タブレットサイズに設定
      ...
      // ボタンがクリック可能
      const ironOreNode = page.getByRole("button", { name: /鉄鉱石/ });
      await ironOreNode.click();
      await expect(page.getByRole("button", { name: /☑\s*鉄鉱石/ })).toBeVisible();
    });

    test("8.3 視覚的フィードバック", async ({ page }) => {
      // ノードをホバー（視覚的変化の確認）
      const ironOreNode = page.getByRole("button", { name: /鉄鉱石/ });
      ...
      const phase2Button = page.getByRole("button", { name: /▶\s*Phase 2:/ });
      await phase2Button.click();
      await expect(page.getByRole("button", { name: /▼\s*Phase 2:/ })).toBeVisible();
    });
  });

  test.describe("9. エッジケースとエラーハンドリング", () => {
    test("9.1 非常にシンプルなレシピ", async ({ page }) => {
      // 鉄インゴット（シンプルなレシピ）を選択
      await page.getByTestId("recipe-button-1101").click();
      await page.getByTestId("roadmap-tab").click();

      // エラーなく表示される
      await expect(page.getByRole("heading", { name: /建設ロードマップ/i })).toBeVisible();

      // シンプルなレシピなのでフェーズ数が少ない（2-3フェーズ程度）
      const phaseButtons = await page.getByRole("button", { name: /▶|▼/ }).all();
    ...
    test("9.2 非常に複雑なレシピ", async ({ page }) => {
      ...
      for (let i = 2; i <= 20; i++) {
        const header = page.getByTestId(`phase-header-${i}`);
  ...
  test.describe("10. 多言語対応", () => {
    ...
    test.skip("10.2 英語表示", async ({ page }) => {
      ...
      // 英語のUIテキストが表示される
      await expect(page.getByRole("heading", { name: /Building Roadmap/i })).toBeVisible();
  ...
  test.describe("11. 統合テスト", () => {
    test("11.1 計算設定変更時の動作", async ({ page }) => {
      await page.getByTestId("recipe-button-1801").click();
      await page.getByTestId("roadmap-tab").click();
      ...
      // 設定を変更（増産剤を変更）
      const proliferatorButtons = await page.locator('[data-testid^="proliferator-"]').all();
