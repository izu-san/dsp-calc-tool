...
export function resolveMachineByRank(
  ...
  switch (recipe.Type) {
    case "Smelt":
      targetId = machineRank === "arc" ? ids[0] : machineRank === "plane" ? ids[1] : ids[2];
      break;
    case "Assemble":
      targetId =
        machineRank === "mk1"
    ...
    case "Chemical":
      ...
      break;
    case "Research":
      targetId = machineRank === "standard" ? ids[0] : ids[1]; // matrixLab/self-evolution
      break;
    case "Refine":
    case "Particle":
...
export function buildRecipeTree(
  ...
  const totalBeltSpeed = settings.conveyorBelt.speed * settings.conveyorBelt.stackCount;
  const conveyorBelts = calculateConveyorBelts(targetRate, inputs, totalBeltSpeed);
