...
export function calculateMachinePower(
  ...
  if (recipeType === "PhotonGeneration" && photonSettings) {
    const rayTransmissionEfficiency = calculateRayTransmissionEfficiency(
      photonSettings.rayTransmissionEfficiency
    );
    const loss = 1 - rayTransmissionEfficiency;

    // 受信効率を計算
    const receptionEfficiency = calculateReceptionEfficiency(
      loss,
