 ...
 * @returns CSV形式の文字列
 */
export function exportToCSV(data: ExportData, options: CSVExportOptions = {}): string {
  ...
  if (opts.includePowerConsumption) {
    ...
    lines.push(
      `Machines,${formatPower(data.powerConsumption.machines)},${((data.powerConsumption.machines / total) * 100).toFixed(2)}`
    );
    lines.push(
      `Sorters,${formatPower(data.powerConsumption.sorters)},${((data.powerConsumption.sorters / total) * 100).toFixed(2)}`
    );
    lines.push(
      `DysonSphere,${formatPower(data.powerConsumption.dysonSphere)},${((data.powerConsumption.dysonSphere / total) * 100).toFixed(2)}`
    );
