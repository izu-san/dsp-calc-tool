...
describe('proliferator', () => {
  ...
  describe('getEffectiveBonuses', () => {
    ...
    it('増産剤Mk.I（speed mode）のボーナスを計算する', () => {
      ...
      expect(result.effectivePowerIncrease).toBe(0.30); // speed mode: powerIncrease * speedMult
    });
    ...
    it('増産剤Mk.III（production mode）のボーナスを計算する', () => {
      ...
      expect(result.effectiveSpeedBonus).toBe(1.00);
      expect(result.effectivePowerIncrease).toBe(1.50); // production mode: powerIncrease * prodMult
    });
    ...
    it('乗数が適用される（production: 2x, speed: 2x）', () => {
      ...
      expect(result.effectiveProductionBonus).toBe(0.40);
      expect(result.effectiveSpeedBonus).toBe(1.00);
      expect(result.effectivePowerIncrease).toBe(1.40);
    });
    ...
    it('nullish乗数がデフォルト1として扱われる', () => {
      ...
      expect(result.effectiveProductionBonus).toBe(0.125);
      expect(result.effectiveSpeedBonus).toBe(0.25);
      expect(result.effectivePowerIncrease).toBe(0.30);
    });
  });
  ...
  describe('getSpeedAndProductionMultipliers', () => {
    it('speed modeの場合、speedMultiplierが適用される', () => {
      ...
      expect(result.speedMultiplier).toBe(1.50);
    ...
    it('カスタム乗数が適用される', () => {
      ...
      expect(result.speedMultiplier).toBe(2.00);
      expect(result.productionMultiplier).toBe(1);
    });
