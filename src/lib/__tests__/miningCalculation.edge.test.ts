...
describe('miningCalculation edge cases', () => {
  ...
  it('returns zeroes when game data missing', async () => {
    const res = calculateMiningRequirements({ rawMaterials: new Map([[1001, 60]]) } as any, 1.0, 'Advanced Mining Machine', 100, null);
    expect(res.rawMaterials).toEqual([]);
    expect(res.totalMiners).toBe(0);
  });
  ...
  it('handles Advanced Mining Machine: workSpeedMultiplier and powerMultiplier fallbacks', async () => {
    ...
    const r1 = calculateMiningRequirements(result, 1.0, 'Advanced Mining Machine', 100, mockGameData);
    ...
    const r2 = calculateMiningRequirements(result, 1.0, 'Advanced Mining Machine', 123 as any, mockGameData);
  ...
  it('computes orbital collectors for Hydrogen with miningSpeedBonus applied', () => {
    ...
    const r = calculateMiningRequirements(result, 2.0, 'Advanced Mining Machine', 100, mockGameData);
