...
describe("photonGenerationCalculation", () => {
  describe("calculateRayTransmissionEfficiency", () => {
    it("研究Lv 0-6 のテーブル値を返す", () => {
      expect(calculateRayTransmissionEfficiency(0)).toBeCloseTo(0.3, 2); // 30%
  ...
  describe("getPhotonGenerationRate", () => {
    it("重力子レンズなし、連続受信100%の場合", () => {
      ...
      expect(rate.productionRate).toBeCloseTo(0.1, 2); // 個/秒
    });
    ...
    it("重力子レンズあり、連続受信100%の場合", () => {
      ...
      expect(rate.productionRate).toBeCloseTo(0.2, 2); // 個/秒 (2倍)
    });
    ...
    it("増産剤Mk.III使用の場合 (+100%速度)", () => {
      ...
      expect(rate.productionRate).toBeCloseTo(0.2, 2); // 0.10 * 2.0
    });
    ...
    it("重力子レンズあり、増産剤Mk.III使用の場合", () => {
      ...
      expect(rate.productionRate).toBeCloseTo(0.4, 2); // 0.20 * 2.0
    });
  });
  ...
  describe("統合テスト: 実際の使用ケース", () => {
    it("ケース1: 初期状態（研究Lv0、重力子レンズなし、増産剤なし）", () => {
      ...
      expect(efficiency).toBeCloseTo(0.3); // 30%効率
      expect(loss).toBeCloseTo(0.7); // 70%損失
    ...
    it("ケース2: 研究Lv50、重力子レンズあり、増産剤Mk.III", () => {
      ...
      expect(rate.productionRate).toBeCloseTo(0.4); // 重力子レンズ+増産剤で4倍
    ...
    it("ケース3: 最悪効率（研究Lv0、連続受信0%）vs 最良効率（研究Lv100、連続受信100%）", () => {
      ...
      const worstPower = calculateRequiredPower(
        worstRate.theoreticalPower,
        worstReceptionEfficiency
      );
