...
import { calculatePowerGeneration, getActualOutput } from "../powerGenerationCalculation";
...
describe("calculatePowerGeneration", () => {
  ...
  describe("後半テンプレート", () => {
    ...
    it("燃料消費量が正しく計算される", () => {
      ...
      expect(result.generators[0].fuelConsumptionRate).toBeCloseTo(expectedConsumption, 5);
    });
  });
  ...
  describe("Artificial Starの燃料別出力", () => {
    ...
    it("ストレンジ物質対消滅燃料棒使用時は144MW", () => {
      ...
      const output = getActualOutput(artificialStar, strangeAnnihilationFuelRod);
...
describe("getActualOutput", () => {
  ...
  it("Artificial Starは燃料によって出力が変わる", () => {
    ...
    const antimatterOutput = getActualOutput(artificialStar, FUEL_ITEMS.antimatterFuelRod);
    expect(antimatterOutput).toBe(72000);

    const strangeOutput = getActualOutput(artificialStar, FUEL_ITEMS.strangeAnnihilationFuelRod);
...
describe("増産剤の適用", () => {
  describe("人工恒星以外の発電設備", () => {
    it("増産剤の追加生産ボーナスが燃料エネルギーに適用される", () => {
      ...
      expect(result.generators[0].actualFuelEnergy).toBeCloseTo(600 * 1.25, 1);
    });
    ...
    it("増産剤なしの場合は基本値が使われる", () => {
      const result = calculatePowerGeneration(15000, "lateGame", "miniFusion", null, 0, 0);
...
describe("手動選択ロジック", () => {
  describe("発電設備の手動選択", () => {
    it("手動で人工恒星を選択すると、テンプレートに関係なく人工恒星が使用される", () => {
      // 序盤テンプレートでも人工恒星を手動選択
      const result = calculatePowerGeneration(100000, "earlyGame", "artificialStar");
    ...
    it("手動でミニ核融合発電所を選択すると使用される", () => {
      const result = calculatePowerGeneration(15000, "earlyGame", "miniFusion");
    ...
    it("手動で火力発電所を選択すると使用される", () => {
      const result = calculatePowerGeneration(5000, "endGame", "thermalPlant");
  ...
  describe("自動選択（手動選択なし）", () => {
    it("発電設備も燃料も自動の場合、テンプレートに基づいて選択される", () => {
      const result = calculatePowerGeneration(100000, "earlyGame", null, null);
