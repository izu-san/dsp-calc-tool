...
import type { PlanInfoForValidation, ValidationResult, ImportError, ImportWarning } from '../../types/import';
 ...
 * @returns 検証結果
 */
export function validatePlanInfo(planInfo: PlanInfoForValidation, gameData: GameData): ValidationResult {
  ...
  if (!planInfo.recipeSID) {
    ...
    if (!recipe) {
      errors.push({ type: 'validation', message: `Recipe with SID ${planInfo.recipeSID} not found in game data.` });
  ...
  if (planInfo.targetQuantity === undefined || planInfo.targetQuantity <= 0) {
    warnings.push({ type: 'partial_data', message: 'Target quantity is invalid or missing, defaulting to 1.' });
