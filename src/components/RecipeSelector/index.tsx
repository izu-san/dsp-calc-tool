...
export function RecipeSelector({ recipes, onRecipeSelect, selectedRecipeId }: RecipeSelectorProps) {
  ...
  const filteredRecipes = useMemo(() => {
    return recipes.filter(recipe => {
      ...
      if (debouncedSearchQuery) {
        ...
        const inputMatch =
          recipe.Items?.some(item => item.name?.toLowerCase().includes(query)) || false;

        // Search in output items
        const outputMatch =
          recipe.Results?.some(result => result.name?.toLowerCase().includes(query)) || false;
  ...
  const getCategoryInfo = useMemo(
    () =>
      (category: string): { iconId?: number; label: string } => {
  ...
  return (
    <div data-testid="recipe-list" className="space-y-4">
              ...
              {searchSuggestions.map((suggestion, index) => (
                ...
                >
                  <span className="flex items-center gap-2">
                    <svg
                      className="w-4 h-4 text-space-400 group-hover:text-neon-cyan transition-colors"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {categories.map(category => {
          ...
          return (
            ...
            >
              {categoryInfo.iconId && (
                <ItemIcon itemId={categoryInfo.iconId} alt={categoryInfo.label} size={20} />
              )}
              {categoryInfo.label}
            </button>
          );
      ...
      {/* Results Count */}
      <div data-testid="recipe-search-results-count" className="text-sm text-space-200">
        <span className="text-neon-cyan font-semibold">{filteredRecipes.length}</span>{" "}
        {filteredRecipes.length !== 1 ? t("recipes") : t("recipe")} {t("found")}
