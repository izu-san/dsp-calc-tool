...
function computeResultBySettings(settings?: any, baseResult?: any) {
  ...
  return {
    rootNode: {
      ...
      bottlenecks: [],
      proliferator: {
        type: settings?.proliferator?.type ?? "none",
        mode: settings?.proliferator?.mode ?? "speed",
      },
      children: [
        {
          nodeId: "child1",
...
describe("WhatIfSimulator", () => {
  ...
  it("シナリオの切り替え（toggle）が動作する", async () => {
    ...
    const toggleBtn =
      (card as HTMLElement).closest("button") ??
      (cardContainer as HTMLElement).querySelector("button");
  ...
  it("すべてのシナリオが適用済みの場合の表示（クイックアクションがすべて無効化される）", async () => {
    ...
    const maxProliferatorButton = screen
      .getByText("maxProliferator")
  ...
  it("isScenarioAlreadyApplied関数が正しく動作する", async () => {
    ...
    if (maybeCard) {
      const cardContainer = maybeCard.closest("div") as HTMLElement;
      const applyBtn = within(cardContainer)
        .getByText("apply")
        .closest("button") as HTMLButtonElement;
