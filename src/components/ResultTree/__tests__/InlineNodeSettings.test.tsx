...
describe("InlineNodeSettings", () => {
  ...
  it("isExpanded=false の場合、何も表示されない", () => {
    render(<InlineNodeSettings node={mockNode} isExpanded={false} onToggle={mockToggle} />);

    expect(screen.queryByText("useCustomSettings")).not.toBeInTheDocument();
  });

  it("isExpanded=true の場合、設定パネルが表示される", () => {
    render(<InlineNodeSettings node={mockNode} isExpanded={true} onToggle={mockToggle} />);
  ...
  it("productive=false の場合、production モードが無効化される", () => {
    render(<InlineNodeSettings node={mockSmeltNode} isExpanded={true} onToggle={mockToggle} />);
