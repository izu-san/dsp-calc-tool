    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-gray-800 rounded-lg shadow-xl p-8">
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">{t("errorOccurred")}</h1>
          <p className="text-gray-400">{t("errorApology")}</p>
        </div>

        {error && (
          <div className="mb-6 bg-gray-900 rounded-lg p-4">
            <h2 className="text-lg font-semibold text-red-400 mb-2">{t("errorDetails")}</h2>
            <pre className="text-sm text-gray-300 overflow-x-auto">{error.toString()}</pre>
        ...
        <div className="mt-6 p-4 bg-yellow-900 border border-yellow-700 rounded-lg">
          <p className="text-sm text-yellow-200">{t("errorTip")}</p>
        </div>
      </div>
    </div>
  );
