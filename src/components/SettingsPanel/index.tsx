...
export function SettingsPanel() {
  ...
  const hasAlternatives =
    selectedRecipe && data && calculationResult
      ? (() => {
    ...
    ? (() => {
        ...
        const hasItem = (node: RecipeTreeNode): boolean => {
          if (node.recipe) {
            ...
            if (
              node.recipe.Items &&
              node.recipe.Items.some(item => item.id === CRITICAL_PHOTON_ID)
            ) {
              return true;
            }
            if (
              node.recipe.Results &&
              node.recipe.Results.some(item => item.id === CRITICAL_PHOTON_ID)
            ) {
              return true;
            }
          }
  ...
  return (
    <div data-testid="settings-panel" className="space-y-4">
      <div>
        ...
        <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 mb-3 hover:border-space-500 transition-colors stagger-item hover-lift">
          <h4 className="text-md font-semibold text-space-100 mb-2">{t("proliferator")}</h4>
          <ProliferatorSettings />
        </div>

        {/* Machine Rank Settings */}
        <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 mb-3 hover:border-space-500 transition-colors stagger-item hover-lift">
          <h4 className="text-md font-semibold text-space-100 mb-2">{t("machineRank")}</h4>
          <MachineRankSettings />
        </div>

        {/* Conveyor Belt Settings */}
        <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 mb-3 hover:border-space-500 transition-colors stagger-item hover-lift">
          <h4 className="text-md font-semibold text-space-100 mb-2">{t("conveyorBelt")}</h4>
          <ConveyorBeltSettings />
        </div>

        {/* Photon Generation Settings - Only show when Critical Photon is in the production chain */}
        {hasCriticalPhoton && (
          <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 mb-3 hover:border-space-500 transition-colors animate-fadeInScale hover-lift">
            <h4 className="text-md font-semibold text-space-100 mb-2">{t("photonGeneration")}</h4>
        ...
        {hasAlternatives && (
          <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 mb-3 hover:border-space-500 transition-colors animate-fadeInScale hover-lift">
            <h4 className="text-md font-semibold text-space-100 mb-2">{t("alternativeRecipes")}</h4>
        ...
        {selectedRecipe && (
          <div className="bg-dark-800/45 rounded-md border border-space-700/70 p-3 hover:border-space-500 transition-colors animate-fadeInScale hover-lift">
            <WhatIfSimulator />
          </div>
        )}
      </div>
