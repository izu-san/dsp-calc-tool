...
export function TemplateSelector() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<keyof typeof SETTINGS_TEMPLATES | null>(
    null
  );
            ...
            <div className="bg-dark-700/95 backdrop-blur-md border-2 border-neon-purple/40 rounded-xl shadow-[0_0_30px_rgba(168,85,247,0.3)] max-w-md w-full p-6 animate-fadeInScale">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-space-200">{t(`${selectedTemplate!}Desc`)}</p>
                </div>
              </div>
              ...
              <div className="bg-dark-800/50 border border-neon-blue/30 backdrop-blur-sm rounded-lg p-4 mb-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-neon-cyan">
                    ...
                    {currentTemplate.settings.conveyorBelt.stackCount > 1 &&
                      ` (${currentTemplate.settings.conveyorBelt.stackCount} ${t("stacks")})`}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-space-300">{t("sorter")}:</span>
                  <span className="font-medium text-neon-cyan">
                    {currentTemplate.settings.sorter.tier === "pile"
                      ? t("pilingSorter")
                      : `Mk.${currentTemplate.settings.sorter.tier.toUpperCase().replace("MK", "")}`}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-neon-cyan">
                    {currentTemplate.settings.miningSpeedResearch}% (+
                    {currentTemplate.settings.miningSpeedResearch - 100}%)
                  </span>
                </div>
              </div>
