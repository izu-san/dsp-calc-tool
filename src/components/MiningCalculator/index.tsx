  ...
  return (
    <div className="bg-dark-700/50 backdrop-blur-sm rounded-xl p-6 border border-neon-yellow/30 shadow-[0_0_20px_rgba(255,215,0,0.2)] space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-space-200">{t("calculateMiningMachinesNeeded")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label
            className="block text-sm font-medium text-neon-blue mb-2 flex items-center gap-2"
            data-testid="miningSpeedResearch"
          >
            <span>🔬</span>
          ...
          <div className="text-xs text-space-300 mt-1">{t("miningResearchHint")}</div>
        </div>

        {/* Mining Machine Type */}
        <div>
          <label
            className="block text-sm font-medium text-neon-green mb-2 flex items-center gap-2"
            data-testid="machineType"
          >
            <span>🏭</span>
            {t("machineType")}
          </label>
          ...
          >
            <option value="Mining Machine" style={{ backgroundColor: "#1E293B", color: "#FFFFFF" }}>
              {t("miningMachine")}
            </option>
            <option
              value="Advanced Mining Machine"
              style={{ backgroundColor: "#1E293B", color: "#FFFFFF" }}
            >
              {t("advancedMiningMachine")}
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          <input
            ...
            data-has-gradient={
              miningSettings.machineType === "Advanced Mining Machine" ? "true" : "false"
            }
          />
      ...
      >
        ...
        {miningCalc.totalOrbitalCollectors > 0 && (
          ...
          >
            <div
              className="text-sm text-space-300 mb-1 flex items-center gap-2"
              data-testid="orbitalCollectors"
            >
              <span>🚀</span>
              {t("orbitalCollectors")}
            </div>
            ...
            <div className="text-xs text-neon-blue mt-1">{t("forHydrogenDeuterium")}</div>
          </div>
        )}
        ...
        >
          ...
          <div className="text-3xl font-bold text-white drop-shadow-[0_0_8px_rgba(168,85,247,0.6)]">
            {miningSettings.workSpeedMultiplier > 100
              ? `${((miningSettings.workSpeedMultiplier / 100) ** 2).toFixed(2)}x`
              : "1.0x"}
          </div>
          <div className="text-xs text-neon-purple mt-1">
            {miningSettings.workSpeedMultiplier > 100
              ? `${miningSettings.workSpeedMultiplier}% ${t("speed")} = ${((miningSettings.workSpeedMultiplier / 100) ** 2 * 100).toFixed(0)}% ${t("power")}`
              : t("standardPower")}
          </div>
        </div>
      </div>
      ...
      {/* Detailed Breakdown */}
      <div data-testid="mining-calculator-material-breakdown">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-white">{material.itemName}</div>
                  ...
                  <div className="text-xs text-space-400">
                    ...
                    {!isOrbitalOnly && material.workSpeedMultiplier > 100 && (
                      <span className="text-neon-orange">
                        {" "}
                        • {material.workSpeedMultiplier}% {t("speed")} (
                        {(material.powerMultiplier * 100).toFixed(0)}% {t("power")})
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  {isOrbitalOnly ? (
                    ...
                    <>
                      <div className="font-bold text-neon-blue text-lg drop-shadow-[0_0_4px_rgba(0,136,255,0.6)]">
                        {formatBuildingCount(material.orbitCollectorsNeeded!)} {t("collectors")}
                        <span data-testid="collectors-label" className="hidden">
                          collectors
                        </span>
                      </div>
                    ...
                    <>
                      <div className="font-bold text-neon-cyan text-lg drop-shadow-[0_0_4px_rgba(0,217,255,0.6)]">
                        ...
                        {material.machineType === "Water Pump"
                          ? t("waterPumps")
                          : t("oilExtractors")}
                        <span data-testid="liquid-equipment-label" className="hidden">
                          liquid-equipment
                        </span>
                      </div>
                      <div className="text-xs text-space-300">
                    ...
                    <>
                      <div className="font-bold text-neon-orange text-lg drop-shadow-[0_0_4px_rgba(255,107,53,0.6)]">
                        {formatBuildingCount(material.veinsNeeded)} {t("veins")}
                        <span data-testid="veins-label" className="hidden">
                          veins
                        </span>
                      </div>
                      <div className="text-xs text-space-300">
                        ~{formatBuildingCount(material.minersNeeded)} {t("minersNeeded")}
                        <span data-testid="minersNeeded-label" className="hidden">
                          minersNeeded
                        </span>
                      </div>
                    </>
      ...
      <div className="text-xs text-space-300 bg-dark-800/50 backdrop-blur-sm border border-neon-cyan/20 rounded-lg p-3 space-y-1">
        <p>
          ⛏️ <strong className="text-neon-orange">{t("veinsNeededLabel")}:</strong>{" "}
          {t("veinsNeededDesc")}
        </p>
        <p>
          📊 <strong className="text-neon-blue">{t("miningSpeedResearch")}:</strong>{" "}
          {t("miningSpeedResearchDesc")}
        </p>
        <p>
          ⚡ <strong className="text-neon-orange">{t("workSpeedAdvanced")}:</strong>{" "}
          {t("workSpeedAdvancedDesc")}
        </p>
        <p>
          🚀 <strong className="text-neon-blue">{t("orbitalCollectors")}:</strong>{" "}
          {t("orbitalCollectorsDesc")}
        </p>
