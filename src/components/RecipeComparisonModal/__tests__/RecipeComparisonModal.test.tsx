...
// stores モック
const machines = new Map<number, any>([[2303, { id: 2303, name: 'Assembler Mk.I', assemblerSpeed: 10000, workEnergyPerTick: 120 }]]);
...
const mockGetMachineForRecipe = vi.fn(() => ({ id: 2303, name: 'Assembler Mk.I', assemblerSpeed: 10000, workEnergyPerTick: 120 }));
...
describe('RecipeComparisonModal', () => {
    ...
    const recipeA: any = { SID: 10, name: 'RecipeA', Type: 'Assemble', Items: [], Results: [{ id: 100, name: 'Item100', count: 1 }], TimeSpend: 60, Explicit: 0, GridIndex: 0, productive: false };
    const recipeB: any = { SID: 20, name: 'RecipeB', Type: 'Assemble', Items: [], Results: [{ id: 100, name: 'Item100', count: 1 }], TimeSpend: 240, Explicit: 0, GridIndex: 0, productive: false };
    ...
    it('machineSpeedMultiplierが0の場合は1に設定される', () => {
        ...
        vi.mocked(vi.importMock('../../../stores/gameDataStore')).useGameDataStore = mockUseGameDataStore;
    ...
    it('増産剤がspeedモードの場合は速度倍率が適用される', () => {
        ...
        vi.mocked(vi.importMock('../../../stores/settingsStore')).useSettingsStore = mockUseSettingsStore;
    ...
    it('効率スコアが負の値の場合は0に設定される', () => {
        // 非常に効率の悪いレシピを作成
        const veryBadRecipe = { ...recipeA, TimeSpend: 3600, Items: [{ id: 1, name: 'Item1', count: 10 }] };
    ...
    it('選択されたレシピがcanBeMined=falseの場合は最初のレシピが選択状態になる', () => {
        ...
        render(<RecipeComparisonModal {...baseProps} canBeMined={false} recipes={[recipeA, recipeB]} />);
    ...
    it('isOpen=falseの場合はuseEffectのクリーンアップが実行される', () => {
        const { rerender } = render(<RecipeComparisonModal {...baseProps} isOpen={true} recipes={[recipeA]} />);
    ...
    it('効率スコアが60以下の場合は赤色が表示される', () => {
        const lowEfficiencyRecipe = { ...recipeA, TimeSpend: 1200, Items: [{ id: 1, name: 'Item1', count: 5 }] };
        render(<RecipeComparisonModal {...baseProps} recipes={[lowEfficiencyRecipe]} />);
        
        const efficiencyElement = screen.getByText(lowEfficiencyRecipe.name)
            .closest('tr')
    ...
    it('効率スコアが60-80の場合は黄色が表示される', () => {
        const mediumEfficiencyRecipe = { ...recipeA, TimeSpend: 180, Items: [{ id: 1, name: 'Item1', count: 2 }] };
        render(<RecipeComparisonModal {...baseProps} recipes={[mediumEfficiencyRecipe]} />);
        
        const efficiencyElement = screen.getByText(mediumEfficiencyRecipe.name)
            .closest('tr')
            ?.querySelector('span.font-bold.text-lg') as HTMLSpanElement;
    ...
    it('選択されたレシピにはチェックマークが表示される', () => {
        ...
        const selectedRecipeRow = screen.getAllByRole('row').find(r => r.textContent?.includes('RecipeA'));
