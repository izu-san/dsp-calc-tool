...
export function RecipeComparisonModal({
  ...
  const analyses = useMemo(() => {
    ...
    for (const recipe of recipes) {
      ...
      const { effectiveSpeedBonus, effectiveProductionBonus } = getEffectiveBonuses(
        settings.proliferator,
        settings.proliferatorMultiplier
      );

      const speedMultiplier =
        settings.proliferator.type !== "none" && settings.proliferator.mode === "speed"
          ? 1 + effectiveSpeedBonus
          : 1;

      const productionMultiplier =
        settings.proliferator.type !== "none" &&
  ...
  const selectedRecipeId =
    settings.alternativeRecipes.get(itemId) ?? (canBeMined ? -1 : recipes[0]?.SID);
  ...
  const modalContent = (
    ...
    >
      <div
        className="bg-dark-700/95 backdrop-blur-md border-2 border-neon-blue/40 rounded-lg ${MODAL_GLOW.blue} max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col relative animate-fadeInScale"
        style={{ zIndex: 100000 }}
      ...
      >
        ...
        <div className="p-6 border-b border-neon-blue/30 flex items-center justify-between bg-dark-800/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white ${TEXT_GLOW.cyan}">
                {t("recipeComparison")}
              </h2>
              <p className="text-sm text-space-200">
                {t("comparingProductionMethods")}:{" "}
                <span className="font-semibold text-neon-cyan">{itemName}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-dark-800/30">
          <div className="mb-4 text-sm text-space-200 bg-neon-blue/10 border border-neon-blue/40 rounded-lg p-3 backdrop-blur-sm">
            📊 {t("comparisonBasedOnProducing")}:{" "}
            <span className="font-semibold text-neon-cyan">
              {BASELINE_PRODUCTION_RATE} {t("itemPerSecond")}
            </span>
          </div>
                ...
                {analyses.map((analysis, index) => {
                  ...
                  return (
                    <tr
                      key={recipeId}
                      className={`
                        ${isSelected ? "bg-neon-cyan/20 border-neon-cyan ${CARD_GLOW.cyanLight}" : "bg-dark-700/30 hover:bg-neon-blue/10"}
                        transition-all
                      `}
                    >
                      {/* Method */}
                      <td className="p-3 border border-neon-blue/20">
                        <div className="flex items-center gap-2">
                          ...
                          {isBest && (
                            <span className="text-neon-yellow text-sm" title={t("mostEfficient")}>
                              ⭐
                            </span>
                          )}
                          {isSelected && (
                          ...
                          <div className="w-20 h-3 bg-space-800 border-2 border-neon-blue/50 rounded-full mt-1 relative overflow-hidden">
                            <div
                              ...
                              style={{
                                ...
                                boxShadow:
                                  analysis.efficiency > 80
                                    ? "0 0 10px rgba(0,255,136,0.8), inset 0 0 5px rgba(255,255,255,0.3)"
                                    : analysis.efficiency > 60
                                      ? "0 0 10px rgba(255,215,0,0.8), inset 0 0 5px rgba(255,255,255,0.3)"
                      ...
                      <td className="p-3 text-center border border-neon-blue/20">
                        <button
                          ...
                          className={`
                            ...
                            ${
                              isSelected
                                ? "bg-neon-cyan/30 border-neon-cyan text-white ${CARD_GLOW.cyan} scale-105"
          ...
          {/* Legend */}
          <div className="mt-6 p-4 bg-neon-purple/10 border border-neon-purple/40 rounded-lg backdrop-blur-sm ${CARD_GLOW.purpleLight}">
            ...
            <ul className="text-sm text-space-200 space-y-1">
              <li>
                • <span className="font-medium text-neon-cyan">40%</span>{" "}
                {t("powerConsumptionLowerIsBetter")}
              </li>
              <li>
                • <span className="font-medium text-neon-cyan">30%</span>{" "}
                {t("machineCountFewerIsBetter")}
              </li>
              <li>
                • <span className="font-medium text-neon-cyan">30%</span>{" "}
                {t("inputComplexityFewerIsBetter")}
              </li>
              <li className="mt-2 text-xs italic text-neon-yellow">
                ⭐ {t("starIndicatesMostEfficient")}
              </li>
            </ul>
          </div>
        </div>
